...
namespace Oceanus.Core.Network
{
    ...
    internal class IMPeerImpl : IMPeer
    {
        ...
        private List<int> mShutdownErrorCodes, mReloginErrorCodes;
        ...
        public IMPeerImpl(string userId, string deviceId, int terminal, string prefix, List<int> shutdownErrorCodes = null, List<int> reloginErrorCodes = null)
        {
            ...
            mReloginErrorCodes = reloginErrorCodes;
            mUserId = userId;
            mDeviceId = deviceId;
            mTerminal = terminal;
            mPrefix = prefix;
        ...
        public void Stop()
        {
            OceanusLogger.info(TAG, mPrefix + " stopped from status {0} to destroyed", mStatus.Get());
        ...
        private void LoginToConnectServer(string loginUrl)
        {
            ...
            string json = JsonMapper.ToJson(loginRequest);// JsonMapper.ToJson(loginRequest);
            ...
            try
            {
                using (HttpWebResponse webresp = (HttpWebResponse)HttpWebRequest.GetResponse())
                {
                    ...
                    {
                        throw new CoreException(CoreClientErrorCodes.ERROR_NETWORK_LOGIN_FAILED, mPrefix + ": Login status code failed, " + (int)webresp.StatusCode);
                    }
                    string responseStr = SafeUtils.StreamToString(webresp.GetResponseStream(), Encoding.UTF8);
                    if (responseStr == null || responseStr.Length == 0)
                        throw new CoreException(CoreClientErrorCodes.ERROR_NETWORK_LOGIN_FAILED, mPrefix + ": Login response is empty");
                    ...
                    {
                        throw new CoreException(CoreClientErrorCodes.ERROR_JSON_PARSE_FAILED, mPrefix + ": Parse login responseStr json failed, " + e.Message + ":" + responseStr);
                    }
            ...
            {
                //请求失败
                throw new CoreException(CoreClientErrorCodes.ERROR_NETWORK_LOGIN_FAILED, mPrefix + ": Login failed, " + e.Message);
            }
        }
        private void ConnectToServer(string host, int port, string token)
        {
            ...
            {
                if(OnPeerReceivedDataEvents != null)
            ...
            {
                if(OnPeerReceivedMessageEvents != null)
        ...
        private void HandleOnPeerDisconnected(IMChannel channel, int code)
        {
            ...
            if(channel == mChannel)
            {
                ...
                if(mStatus.Get() == STATUS_DESTROYED || (mShutdownErrorCodes != null && mShutdownErrorCodes.Contains(code)))
                {
                    mStatus.Set(STATUS_DESTROYED);
                    SafeUtils.SafeCallback(mPrefix + ": HandleOnPeerDisconnected call OnPeerShuttedDownEvents code " + code + " will NOT reconnect", () =>
                    {
                        if(OnPeerShuttedDownEvents != null)
                ...
                } else if (mStatus.Get() != STATUS_DISCONNECTED)
                {
                    ...
                    string reloginStr = "";
                    if(mReloginErrorCodes != null && mReloginErrorCodes.Contains(code))
                    {
                        reloginStr = " Need relogin";
                    }
                    SafeUtils.SafeCallback(mPrefix + ": HandleOnPeerDisconnected call OnPeerDisconnectedEvents code " + code + reloginStr, () =>
                    {
                        if(OnPeerDisconnectedEvents != null)
        ...
        public string SendInvocation(string service, string className, string method, object[] args, OnIMResultReceived onIMResultReceivedMethod, int sendTimeoutSeconds, string clientId)
        {
            checkConnectionStatus(onIMResultReceivedMethod);
            if(clientId == null)
            {
            ...
            return resultAction.Id;
        }
        public string SendData(object content, string contentType, OnIMResultReceived onIMResultReceivedMethod, int sendTimeoutSeconds)
        {
            checkConnectionStatus(onIMResultReceivedMethod);
        ...
        private void checkConnectionStatus(OnIMResultReceived onIMResultReceivedMethod)
        {
            if (mChannel == null || mChannel.Status() != IMConstants.CHANNEL_STATUS_CONNECTED)
            {
                if (onIMResultReceivedMethod != null)
                    onIMResultReceivedMethod(new IMResult()
                    {
                        //ForId = Guid.NewGuid().ToString("N"),
                        Code = CoreClientErrorCodes.ERROR_NETWORK_DISCONNECTED,
        ...
        public string GetChannelAttachment()
        {
            if (mChannel != null)
                return mChannel.GetAttachment();
            return null;
        }
        public T GetChannelAttachment<T>()
        {
            if (mChannel != null)
            {
                return mChannel.GetAttachment<T>();
            }
            return default(T);
        }
